...
const getExpoExtra = (): ExpoExtra | undefined => {
  ...
  const maybe =
    ...
    ((Constants as unknown as { manifest2?: { extra?: unknown } }).manifest2?.extra as
      | ExpoExtra
      | undefined) ??
    ((Constants as unknown as { manifest?: { extra?: unknown } }).manifest?.extra as ExpoExtra | undefined);
  return maybe;
};
...
const getApiBaseUrl = (): string => {
  ...
  const fromAppJson = (appJson as { expo?: { extra?: ExpoExtra } } | undefined)?.expo?.extra?.apiBaseUrl;
